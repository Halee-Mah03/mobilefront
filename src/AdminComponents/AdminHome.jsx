...
const AdminHome = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/auth/admin-name')
      .then((response) => {
  ...
  const clientCount = () => {
    axios.get('http://localhost:3000/auth/client_count')
    .then(result => {
      if(result.data.Status) {
        setClientTotal(result.data.Result[0].client)
      }
    })
  }
  ...
  const balanceCount = () => {
    axios.get('http://localhost:3000/auth/balance_count')
    .then(result => {
      if(result.data.Status) {
        setClientBalanceTotal(result.data.Result[0].balanceOFClient)
      } else {
        alert(result.data.Error)
      }
    })
...
<div className="bg-white-300 font-[sans-serif]">
  ...
  <path d="M5.082 14.254a8.287 8.287 0 0 0-1.308 5.135 9.687 9.687 0 0 1-1.764-.44l-.115-.04a.563.563 0 0 1-.373-.487l-.01-.121a3.75 3.75 0 0 1 3.57-4.047ZM20.226 19.389a8.287 8.287 0 0 0-1.308-5.135 3.75 3.75 0 0 1 3.57 4.047l-.01.121a.563.563 0 0 1-.373.486l-.115.04c-.567.2-1.156.349-1.764.441Z" />
              ...
              <h4 className="text-white-500 text-sm leading-relaxed">{adminTotal}</h4>
            </div>
          </div>
  ...
  <path d="M4.5 6.375a4.125 4.125 0 1 1 8.25 0 4.125 4.125 0 0 1-8.25 0ZM14.25 8.625a3.375 3.375 0 1 1 6.75 0 3.375 3.375 0 0 1-6.75 0ZM1.5 19.125a7.125 7.125 0 0 1 14.25 0v.003l-.001.119a.75.75 0 0 1-.363.63 13.067 13.067 0 0 1-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 0 1-.364-.63l-.001-.122ZM17.25 19.128l-.001.144a2.25 2.25 0 0 1-.233.96 10.088 10.088 0 0 0 5.06-1.01.75.75 0 0 0 .42-.643 4.875 4.875 0 0 0-6.957-4.611 8.586 8.586 0 0 1 1.71 5.157v.003Z" />
              ...
              <h4 className="text-gray-500 text-sm leading-relaxed">{employeeTotal}</h4>
            </div>
    </div>
  ...
  <path d="M4.5 6.375a4.125 4.125 0 1 1 8.25 0 4.125 4.125 0 0 1-8.25 0ZM14.25 8.625a3.375 3.375 0 1 1 6.75 0 3.375 3.375 0 0 1-6.75 0ZM1.5 19.125a7.125 7.125 0 0 1 14.25 0v.003l-.001.119a.75.75 0 0 1-.363.63 13.067 13.067 0 0 1-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 0 1-.364-.63l-.001-.122ZM17.25 19.128l-.001.144a2.25 2.25 0 0 1-.233.96 10.088 10.088 0 0 0 5.06-1.01.75.75 0 0 0 .42-.643 4.875 4.875 0 0 0-6.957-4.611 8.586 8.586 0 0 1 1.71 5.157v.003Z" />
              ...
              <h4 className="text-gray-500 text-sm leading-relaxed">{clientTotal}</h4>
            </div>
    </div>
    <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-all">
            <div className="p-8">
